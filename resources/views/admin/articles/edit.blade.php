        ...
        <div class="col-xs-12">
            <section class="panel">
                <header class="panel-heading">
                    <h2 class="panel-title">Nhập thông tin cần sửa</h2>
                </header>
                <div class="panel-body">
                    ...
                    @endif
                    <form class="form-horizontal form-bordered" action="/admin/articles/{{$obj->id}}" method="post">
                        @method('put')
                        @csrf
                        <div class="form-group">
                            <label class="col-md-3 control-label" for="inputDefault">Title</label>
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="inputDefault" name="title" value="{{$obj->title}}">
                                @error('title')
                            ...
                            <div class="col-md-6">
                                <select class="form-control mb-md" name="category_id">
                                    @foreach($categories as $cate)
                                        <option {{$obj->category_id == $cate->id ? 'selected':''}} value="{{$cate->id}}">{{$cate->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <textarea name="content" class="summernote" data-plugin-summernote data-plugin-options='{ "height": 180, "codemirror": { "theme": "ambiance" } }'>{!! $obj->content !!}</textarea>
                                @error('content')
                                <div class="text-danger">* {{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="inputDefault" name="thumbnail" value="{{$obj->thumbnail}}">
                                @error('thumbnail')
                                <div class="text-danger">* {{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <input type="text" class="form-control" id="inputDefault" name="status" value="{{$obj->status}}">
                                @error('status')
                                <div class="text-danger">* {{ $message }}</div>
                                @enderror
                            </div>
